...
- Der separate Passwort-Follow-up-Dispatch erfasst den erfolgreichen Policy- und Settings-Snapshot bereits beim Erstellen der Freigabe und speichert den bereinigten Signaturinhalt mit jedem dauerhaften Entwurf. Er setzt und prueft `SendUsingAccount`/`SentOnBehalfOfName`, sendet nur automatisch, wenn die effektive Follow-up-Identitaet dem beim Sendeversuch der Hauptmail erfassten Absender entspricht, und fuegt die Backend-Signatur nur ein, wenn diese effektive Identitaet zusaetzlich zu `policy.email_signature.user_email` passt. Eine Plain-Text-Quelle erzeugt einen Plain-Text-Follow-up; HTML/RTF erzeugt einen HTML-Follow-up. Schlägt der automatische Versand fehl, zeigt Outlook denselben vorbereiteten Entwurf an, statt eine zweite Nachricht zu erzeugen.
...
- `OnSend` ist die Persistenzgrenze. Outlooks integrierte verzögerte Übermittlung durchläuft dieses Ereignis, bevor die Hauptmail in den Postausgang wechselt. Nur ein `ItemAdd`-Ereignis oder eine Neustartprüfung, die die markierte Hauptmail mit `MailItem.Sent=true` im exakt gespeicherten Ordner findet, bestätigt den Versand. Verzögerter oder Offline-Postausgang bleibt dadurch ausstehend und überlebt einen Outlook-Neustart nach dem Sendeversuch.
...
- `ComposeShareLifecycleController` übernimmt die Passwortmail-Aufbereitung; `PendingPasswordDraftController` übernimmt Persistenz und bestätigten Versand. SMTP-Adressen werden gemeinsam über An, Cc und Bcc dedupliziert. Im Secrets-Modus entsteht ein Einmal-Link pro eindeutiger Adresse. Der Follow-up startet erst nach positiver Bestätigung im exakten Gesendet-Ordner; bei Auto-Send-Fehlern wird derselbe gespeicherte Entwurf zur manuellen Zustellung angezeigt.
...
- Akzeptierter Outlook-Pfad: `/nc-ifb/<profile-secret>/freebusy/<address>.vfb`; das Secret wird lokal erzeugt und nur im DPAPI-geschützten Profilzustand gespeichert
- Anfragen ohne Profil-Secret erhalten `404`
